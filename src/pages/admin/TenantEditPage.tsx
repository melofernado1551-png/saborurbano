  ...
  useEffect(() => {
    if (tenant) {
      ...
      setForm({
        ...
        city: tenant.city || "",
        state: tenant.state || "",
        address: tenant.address ? tenant.address.replace(/,\s*\d+$/, "") : "",
        number: tenant.address ? (tenant.address.match(/,\s*(\d+)$/)  ?.[1] || "") : "",
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      const payload = {
        ...
        address: form.address ? (form.number ? `${form.address}, ${form.number}` : form.address) : null,
